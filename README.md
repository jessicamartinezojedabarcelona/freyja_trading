...
- Por razones de seguridad (evitar enumeración de cuentas existentes), la
  respuesta pública es idéntica si el correo ya estaba registrado o no. Un
  registro repetido con un correo existente **nunca** cambia la contraseña de
  esa cuenta.

### Cuentas independientes y datos

- Cada persona crea su propia cuenta y entra con sus propias credenciales;
  las cuentas no se comparten ni se reutilizan. Inicio de sesión, sesión,
  cierre de sesión y recuperación de contraseña son independientes por
  cuenta: cerrar la sesión, restablecer la contraseña o agotar los intentos
  de inicio de sesión de una cuenta no afecta a las demás.
- Los datos de mercado y los gráficos (catálogo, `/api/v1/catalog/*`,
  `/api/v1/capabilities`) son comunes a todas las cuentas. Los datos
  privados de cada persona (por ejemplo, los `ExecutionContext` de
  `/api/v1/execution-contexts`) están asociados a su propietario y ninguna
  cuenta ve los de otra.
- Todas las cuentas tienen el mismo acceso: no hay roles ni permisos
  distintos. Un panel para administrar cuentas es una tarea posterior y no
  condiciona el registro.

### Probar el registro

**En local**, con backend y frontend arrancados (§11):

1. Abre `http://localhost:4200/register`, escribe un correo y una contraseña
   de 12 a 128 caracteres, confírmala y envía. Debe aparecer «Tu cuenta ha
   sido creada. Ya puedes iniciar sesión.».
2. Ve a `/login` e inicia sesión con esas credenciales.
3. Repite el registro con el mismo correo y otra contraseña: la respuesta es
   la misma y, al iniciar sesión, solo sirve la contraseña original.

Pruebas automatizadas: desde `backend/`,
`uv run pytest tests/api/test_auth_multi_user.py tests/api/test_auth_email_flows.py`;
desde `frontend/`, `npm run test:ci`.

**En producción**, tras un despliegue (manual, §19): abre `/register` del
frontend publicado y crea **tu propia cuenta real**. No se crean cuentas de
ejemplo ni se comparten credenciales. La recuperación de contraseña sigue
sin ser operativa online mientras no haya un proveedor SMTP (más abajo).
